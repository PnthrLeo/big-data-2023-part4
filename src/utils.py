...
def get_X_y(df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.Series]:
    ...
    if not config['data']['feature_names']:
        ...
        X = df[config['data']['feature_names'].split(',')]
    
    return X, y

def split_for_validation(X: pd.DataFrame, y: pd.Series) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
...
def set_right_dtypes(df: pd.DataFrame) -> pd.DataFrame:
    dict_dtypes = dict(config['data']['dtypes'])
